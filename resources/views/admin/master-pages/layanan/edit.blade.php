            ...
            <form action="{{ route('admin.layanan.update', ['layanan' => $layanan->slug]) }}" method="post"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama"
                        id="nama" value="{{ old('nama', $layanan->nama) }}">
                    @error('nama')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug"
                        id="slug" readonly value="{{ old('slug', $layanan->slug) }}">
                    @error('slug')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <img id="displayedIcon" class="img-fluid w-50"
                        src="{{ old('icon', asset('storage/' . $layanan->icon)) }}" alt="icon yang Dipilih">
                </div>
                <label for="" class="form-label">icon</label>
                <div class="input-group mb-3">
                    <input type="file" class="form-control @error('icon') is-invalid @enderror" name="icon"
                        id="icon" accept="image/*" value="{{ old('icon', $layanan->icon) }}">
                ...
                <div class="mb-3">
                    ...
                    <img id="displayedImage" class="img-fluid w-50"
                        src="{{ old('gambar', asset('storage/' . $layanan->gambar)) }}" alt="Gambar yang Dipilih">
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">gambar</label>
                    <input type="file" class="form-control @error('gambar') is-invalid @enderror" name="gambar"
                        id="gambar" id="gambar" accept="image/*" value="{{ old('gambar', $layanan->gambar) }}">
                    @error('gambar')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <a class="btn btn-warning"
                    href="{{ route('admin.layanan', ['kategoriLayanan' => $layanan->kategoriLayanan]) }}">Kembali</a>
