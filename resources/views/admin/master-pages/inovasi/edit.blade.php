@extends('admin.pages.main', ['sloot' => 'Edit Artikel'])
@push('link-css-admin')
...
@section('content-admin')
    <div class="card">
        <div class="card-body">
            <h1 class="text-center text-capitalize fw-bold">Edit Inovasi</h1>
            <form action="{{ route('admin.inovasi.update', ['inovasi' => $inovasi->slug]) }}" method="post"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('judul') is-invalid @enderror" name="judul"
                        id="judul" value="{{ old('judul', $inovasi->judul) }}">
                </div>
                <div class="mb-3">
                    <label for="slug" class="form-label">slug</label>
                    <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug"
                        id="slug" readonly value="{{ old('slug', $inovasi->slug) }}">
                </div>
                <div class="mb-3">
                    <p class="d-none" id="text-preview">preview gambar</p>
                    <img id="displayedImage" class="img-fluid w-50"
                        src="{{ old('gambar', asset('storage/' . $inovasi->gambar)) }}" alt="Gambar yang Dipilih">
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">Gambar</label>
                    <input type="file" class="form-control @error('gambar') is-invalid @enderror" name="gambar"
                        id="gambar" id="gambar" accept="image/*" value="{{ old('gambar', $inovasi->gambar) }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input id="deskripsi" placeholder="masukkan deskripsi berita" type="hidden" name="deskripsi"
                        value="{{ old('deskripsi', $inovasi->deskripsi) }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('link_yt') is-invalid @enderror" name="link_yt"
                        id="link_yt" value="{{ old('link_yt',$inovasi->link_yt) }}">
                </div>
                <div class="mb-3">
                    ...
                    @foreach ($kategoriKonten as $item)
                        <div class="col-md-6">
                            <input type="checkbox" name="kategori[]" value="{{ $item->id }}" id="{{ $item->id }}"
                                @if (in_array($item->id, $inovasi->kategori_konten->pluck('id')->toArray())) checked @endif>
                            <label for="{{ $item->id }}"> {{ $item->nama }}</label>
                        </div>
                    @endforeach
                </div>
                <a class="btn btn-warning" href="{{ route('admin.inovasi.index') }}">Kembali</a>
