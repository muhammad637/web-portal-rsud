...
@section('content-admin')
    <div class="card">
        <div class="card-body">
            <h1 class="text-center text-capitalize fw-bold">form upload Pelayanan Layanan Unggulan</h1>
            <form action="{{ route('admin.unggulan.update', ['unggulan' => $unggulan->slug]) }}" method="post"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama"
                        id="nama" value="{{ old('nama', $unggulan->nama) }}">
                    @error('nama')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug"
                        id="slug" readonly value="{{ old('slug', $unggulan->slug) }}">
                    @error('slug')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <img id="displayedIcon" class="img-fluid w-50"
                        src="{{ old('icon', asset('storage/' . $unggulan->icon)) }}" alt="icon yang Dipilih">
                </div>
                <label for="" class="form-label">icon</label>
                <div class="input-group mb-3">
                    <input type="file" class="form-control @error('icon') is-invalid @enderror" name="icon"
                        id="icon" accept="image/*" value="{{ old('icon', $unggulan->icon) }}">
                ...
                <div class="mb-3">
                    ...
                    <img id="displayedImage" class="img-fluid w-50"
                        src="{{ old('gambar', asset('storage/' . $unggulan->gambar)) }}" alt="Gambar yang Dipilih">
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">gambar</label>
                    <input type="file" class="form-control @error('gambar') is-invalid @enderror" name="gambar"
                        id="gambar" id="gambar" accept="image/*" value="{{ old('gambar', $unggulan->gambar) }}">
                    @error('gambar')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <input id="deskripsi" placeholder="masukkan deskripsi berita" type="hidden" name="deskripsi"
                        value="{{ old('deskripsi', $unggulan->deskripsi) }}">
                    <trix-editor input="deskripsi"></trix-editor>
                </div>

                <a class="btn btn-warning" href="{{ route('admin.unggulan') }}">Kembali</a>
