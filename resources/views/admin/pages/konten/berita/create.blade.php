        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.berita.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <img id="displayedImage" class="img-fluid w-50" src="" alt="Gambar yang Dipilih">
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">gambar</label>
                    <input type="file" class="form-control @error('gambar') is-invalid @enderror" name="gambar"
                        id="gambar"  id="gambar" accept="image/*">
                </div>
...
@push('link-script-admin')
    <script>
        $(document).ready(function() {
            ...
            //     console.log('testing')
            //     fetch(`{{ route('admin.createSlug') }}?title= ${$('#judul').val()}`)
            //     .then(response => {response.json(); console.log('testing')})
            //     .then(data => slug.value = data.slug)
            // })
